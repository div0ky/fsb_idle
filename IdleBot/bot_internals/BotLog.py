...
class BotLog:
    def __init__(self):
        ...
        # Create formatters
        file_format = logging.Formatter(f'%(asctime)s.%(msecs)03d  |  %(levelname)s  |  %(module)s  |  {full_version}  |  %(message)s',datefmt='%Y-%m-%d | %H:%M:%S')
        console_format = logging.Formatter(f'%(asctime)s.%(msecs)03d  |  %(levelname)s  |  %(module)s  |  {full_version}  |  %(message)s',datefmt='%Y-%m-%d | %H:%M:%S')
